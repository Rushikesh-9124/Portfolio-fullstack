...
export default function HomePage() {
  const [data, setData] = useState({ about: null, skills: [], projects: [], certificates: [] });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchAll = async () => {
      try {
        const [aboutRes, skillsRes, projectsRes, certsRes] = await Promise.allSettled([
          aboutAPI.get(),
          skillsAPI.getAll(),
          projectsAPI.getAll(),
        ...
        setData({
          ...
          certificates: certsRes.status === 'fulfilled' ? certsRes.value.data.data : [],
        });
      } catch (err) {
